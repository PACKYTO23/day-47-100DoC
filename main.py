...
wanted_price: float = 275.00

my_email = "zmssmzx@gmail.com"
password = "lyqi vxqn qfhq usty"
email_juan = "juanfcr11@gmail.com"
up_message = "The price is still not low enough. We'll keep checking for special sales on your equipment! ;)"
down_message = "The price has gone below the $275. It's a great time to get that MF cable! ;)"

with smtplib.SMTP("smtp.gmail.com") as connection:
    connection.starttls()
    connection.login(user=my_email, password=password)

    if price_tag > wanted_price:
        connection.sendmail(
            from_addr=my_email,
            to_addrs=email_juan,
            msg=f"Subject:Roland MIDI Cable ON SIGHT!!! ;P\n\n{up_message}"
        )
    elif price_tag <= wanted_price:
        connection.sendmail(
            from_addr=my_email,
            to_addrs=email_juan,
            msg=f"Subject:Roland MIDI Cable ON SIGHT!!! ;P\n\n{down_message}"
